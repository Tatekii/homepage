...
const appleMusicLink = "https://embed.music.apple.com/cn/album/notd-at-project-glow-2024-dj-mix/1747984504"

const SongEmbed: FC<SongEmbedProps> = ({ src = appleMusicLink }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 w-full max-w-md mx-auto mb-4 px-4 z-10">
            <div className="relative rounded-lg overflow-hidden bg-background/30 backdrop-blur-md border border-chart-1/30 shadow-lg shadow-chart-2/20 transition-all duration-300">
                <div className="flex items-center justify-between px-3 pt-2 pb-1">
                    ...
                    <button 
                        ...
                        className="p-1 rounded-full hover:bg-chart-1/20 transition-colors"
                        aria-label={isCollapsed ? "Expand music player" : "Collapse music player"}
                    >
                        {isCollapsed ? (
                            <ChevronUp className="h-4 w-4 text-chart-1" />
                        ) : (
                            <ChevronDown className="h-4 w-4 text-chart-1" />
                        )}
                    </button>
                </div>
                
                <div className={`transition-all duration-300 origin-top ${isCollapsed ? 'h-0 opacity-0' : 'opacity-100'}`}>
                    <iframe
                        width="100%"
                        height="110"
                        scrolling="no"
